 ...
 */
export async function uploadFileWithMetadata(file: File, docId?: string) {
    try {
        // Validate file
        ...
        // If docId is provided, use it for structure: documents/{docId}/source.pdf
        // Otherwise fallback to old: public/{timestamp}-{filename}
        const fileName = docId
            ? `documents/${docId}/source.pdf`
            : `public/${timestamp}-${file.name}`;
